...
from util.submodel.music_dmm import systemPromptTemplate as musicSystemPrompt

from data.config.config import (
    dmmLLM, edmmLLM, ndmmLLM, musicPlayerDmmLLM, codeBrew,
...
musicSystemPrompt.template.extend(
        [
            "THIS IS THE CURRENT STATUS OF THE MUSIC PLAYER",
            Function(
                musicPlayer.get_playlist
            ),
            Function(
                musicPlayer.get_status
            )
        ]
    )
# ------------------------------------------------ #
...
def dmm(query: str, chatPrompt: Optional[str] = None) -> list[JFunction]:
    ...
    dmmLLM.messages.extend(
        dmmHistoryDb.getLastNMessages(
            n=int(os.getenv("JARVIS_DMM_HISTORY_LIMIT", 20)),
    ...
    def musicPlayerAssistant(query: str):
        def func(query: str):
            musicPlayerDmmLLM.messages = []
            musicPlayerDmmLLM.addMessage(Role.system, musicSystemPrompt.prompt)
            
            response = musicPlayerDmmLLM.run(query)
            ...
            def loop(**a):
                musicPlayer.toggle_loop()
                return "Looping"
            
            def volume(level: int):
                musicPlayer.set_volume(level/100)
                return "Volume set to " + str(level)
            
            def print_text(message: str):
                return "Music Player Assistant: " + message
            print(f"{response = }")
            try:
                r = eval(response)
                print(f"{r = }")
                ...
                functionList.append(
                    JFunction(
                        ...
                        function=Function(
                            print,
                            "error in musicPlayerAssistant : " + str(e),
                        ),
                        priority=1
                    )
                )
